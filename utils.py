...
def get_deltaforce_secrets():
    ...
    result = re.findall(r'<span\s+class="name">(.*?)</span>', response.text)
    ...
    if response.status_code == 200:
        ...
        logger.error(f"获取密码门密码失败，状态码：{response.status_code}")

def getSecrets():
    formatted_date = datetime.datetime.now().strftime("%Y-%m-%d")
    ensure_file_exists("deltaforce_secrets.txt")
    with open("deltaforce_secrets.txt", "r", encoding="utf-8") as f:
        file_text = f.read()
        last_date = file_text.split("\n")[0].strip()
        if last_date != formatted_date:
            formatted_text = get_deltaforce_secrets()
        else:
